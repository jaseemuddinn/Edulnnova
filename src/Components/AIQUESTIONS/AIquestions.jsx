                ...
                switch (type.toLowerCase()) {
                    ...
                    case 'fillups':
                        fetchedQuestions = [
                            ...
                            { question: 'The chemical formula for water is ______.', answer: 'H2O' },
                        ];
                        break;
                    case 'matching':
                        fetchedQuestions = [
                            { question: 'Match the following:', pairs: { '1': 'A', '2': 'B', '3': 'C' }, options: { '1': 'Apple', '2': 'Banana', '3': 'Cherry' } },
                        ];
                        break;
            ...
            {step === 1 && (
                <div className="feature-selection">
                    ...
                    <div className="feature-buttons">
                        <button onClick={() => handleFeatureSelect('explore')}>Explore</button>
                        <button onClick={() => handleFeatureSelect('quick')}>Quick</button>
                    </div>
                </div>
            )}
            ...
            {step === 2 && (
                <div className="topic-selection">
                    <form className="topic-form" onSubmit={handleTopicSubmit}>
                        <h2>Step 1: Choose Your Topic for {feature === 'explore' ? 'Explore' : 'Quick'}</h2>
                        <p>Type your topic:</p>
                        ...
                        <button type="submit">{feature === 'explore' ? 'Next' : 'Start Practice'}</button>
                    </form>
                    <button className="back-button" onClick={handleBack}>Back to Feature Selection</button>
                </div>
            )}

            {step === 3 && (
                <div className="question-type-selection">
                    <h2>Step 2: {feature === 'explore' ? 'Select' : 'Practice'} Question Type</h2>
                    {feature === 'explore' ? (
                        ...
                        <div className="question-type-buttons">
                            ...
                            <button className='ar' onClick={() => {
                                ...
                                setStep(4);
                                setQuestionIndex(0);
                            }}>Assertion Reasoning</button>
                            {/* Other buttons */}
                        </div>
                    )}
                    <button className="back-button" onClick={handleBack}>Back to {feature === 'explore' ? 'Topic Selection' : 'Feature Selection'}</button>
                </div>
            )}

            {step === 4 && (
                <div className="question-section" style={{ height: '100vh' }}>
                    <h2>Practice {questionType.toUpperCase()} for {topic}</h2>
                    {error && <p className="error-message">{error}</p>}
                    {questions.length === 0 ? (
                        <p>No questions available for this type. Please choose a different type.</p>
                    ) : (
                        <>
                            {questionType === 'mcq' && (
                            ...
                            <div className="navigation-buttons">
                                ...
                                {questionIndex < questions.length - 1 && <button onClick={() => setQuestionIndex(questionIndex + 1)}>Next Question</button>}
                                <button onClick={handleBack}>Back to Question Type Selection</button>
                            </div>
                        </>
                    )}
                </div>
